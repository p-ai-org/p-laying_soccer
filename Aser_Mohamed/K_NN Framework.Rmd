...
data_train1 <- data_train[,-1] #just removes the automatic counter of rows

data_test2  <- read.csv("roundtwo.csv") #reads predicitons file (Here put the new data with the predictions)
data_test2  <- data_test[, -1] #just removes the automatic counter of rows
...
data_validation2 <- data_validation[, -1] #just removes the autmoatic counter of rows
```
...
data_validation3 <- data_validation[, -1]
```
...
data_validation4 <- data_validation[, -1]
```
...
data_validation5 <- data_validation[, -1]
```
...
for(i in 1 : (nrow(data_train)/2) ) 
{
knn <- class::knn(train= data_train[, selectedIndices], test = data_test[, selectedIndices], cl= data_train[, outcome], k= i) #calcualtes the predicitons 
